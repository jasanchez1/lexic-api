...
guide_related_guides = Table(
    ...
    Base.metadata,
    Column('guide_id', UUID(as_uuid=True), ForeignKey('guides.id', ondelete="CASCADE"), primary_key=True),
    Column('related_guide_id', UUID(as_uuid=True), ForeignKey('guides.id', ondelete="CASCADE"), primary_key=True),
)
...
class Guide(Base):
    ...
    sections = relationship("GuideSection", back_populates="guide", cascade="all, delete-orphan")
