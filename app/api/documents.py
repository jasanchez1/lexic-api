...
async def upload_documents(
    ...
    if lawyer.user_id != current_user.id and not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Not authorized to upload documents for this lawyer"
        )
    ...
    if "supreme_court_certificate" in document_types and supreme_court_certificate:
        ...
        if supreme_court_certificate.size > 5 * 1024 * 1024:
            raise HTTPException(
                status_code=400,
                detail="Supreme court certificate exceeds 5MB size limit",
            )

        # Check file type
        if supreme_court_certificate.content_type not in [
            "application/pdf",
            "image/jpeg",
            "image/png",
        ]:
            raise HTTPException(
                status_code=400,
                detail="Supreme court certificate must be a PDF, JPG, or PNG file",
            )

        # Check if document already exists
        existing_doc = documents_repository.get_document_by_type(
            db, lawyer_id, "supreme_court_certificate"
        )
        if existing_doc:
            # Delete old file
        ...
        file_data = await storage_service.upload_file(
            supreme_court_certificate, lawyer_id, "supreme_court_certificate"
        )
    ...
    if "university_degree" in document_types and university_degree:
        ...
        if university_degree.size > 5 * 1024 * 1024:
            raise HTTPException(
                status_code=400, detail="University degree exceeds 5MB size limit"
            )

        # Check file type
        if university_degree.content_type not in [
            "application/pdf",
            "image/jpeg",
            "image/png",
        ]:
            raise HTTPException(
                status_code=400,
                detail="University degree must be a PDF, JPG, or PNG file",
            )

        # Check if document already exists
        existing_doc = documents_repository.get_document_by_type(
            db, lawyer_id, "university_degree"
        )
        if existing_doc:
            # Delete old file
            storage_service.delete_file(existing_doc.file_path)
        ...
        # Upload file
        file_data = await storage_service.upload_file(
            university_degree, lawyer_id, "university_degree"
        )
...
async def get_lawyer_documents(
    ...
    if lawyer.user_id != current_user.id and not current_user.is_admin:
        raise HTTPException(
            status_code=403, detail="Not authorized to access documents for this lawyer"
        )
...
@router.put(
    "/admin/lawyers/{lawyer_id}/documents/{document_type}/verify",
    response_model=LawyerDocumentsResponse,
)
